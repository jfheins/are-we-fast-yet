...
{
  private const String RAP_BENCHMARK_MINIFIED =
  ...
  public override bool VerifyResult(object r)
  {
    if (r == null)
    {
      return false;
    ...
    JsonValue result = (JsonValue) r;
    if (!result.IsObject())
    {
      return false;
    }

    if (!(result.AsObject()?.Get("head")?.IsObject() ?? false))
    {
      return false;
    }

    if (!(result.AsObject()?.Get("operations")?.IsArray() ?? false))
    {
      return false;
    }
  ...
  sealed class HashIndexTable
  {
    private readonly int[] hashTable;

    public HashIndexTable()
    {
      this.hashTable = new int[32]; // must be a power of two
    }
    ...
    private int HashSlotFor(string element)
    {
  ...
  * <strong>bool</strong> values can be created using the static factory methods
  ...
  * {@link JsonValue}, strings, primitive numbers, and bool values. To replace an element of an
  ...
  */
  ...
  {
    private readonly Vector<JsonValue> values;
  ...
  sealed class JsonLiteral : JsonValue
  {
  ...
  * {@link JsonValue}, strings, primitive numbers, and bool values. To modify certain values of an
  ...
  * </p>
  * <p>Members can be accessed by their name using {@link #get(String)}.</p>
  * <p>This class is <strong>not supposed to be extended</strong> by clients.</p>
  */
  public sealed class JsonObject : JsonValue
  {
    private readonly Vector<String> names;
    private readonly Vector<JsonValue> values;
    private readonly HashIndexTable table;
    ...
    */
    ...
    private int IndexOf(string name)
    {
      ...
      throw new NotImplementedException(); // Not needed for benchmark
    }
  }
  ...
  public class ParseException : Exception
  {
    ...
    public ParseException(string message, int offset, int line, int column) : base(message + " at " + line + ":" +
      column)
    {
      this.offset = offset;
      this.line = line;
